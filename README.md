...
<head>
  ...
  <title>Geggets Stores — Electronics & Gadgets</title>
...
<body>
  <div class="container">
    ...
    <main id="top">
      <section class="hero" aria-labelledby="hero-heading">
        <div>
          <h1 id="hero-heading">Geggets Stores — আধুনিক ইলেকট্রনিক গ্যাজেট</h1>
          <p>খুঁজে নাও সর্বশেষ গ্যাজেটগুলো — স্মার্টওয়াচ, ইয়ারবাড, RGB লাইট, পোর্টেবল স্পিকার এবং আরও অনেক কিছু।</p>
      ...
      <p class="small" style="margin-top:6px">নীচে জনপ্রিয় ২৫টি গ্যাজেট দেখানো হলো — সার্চ বারের মাধ্যমে দ্রুত খুঁজে নাও (সাজেশনও দেখাবে)।</p>
      <div class="grid" id="product-grid"></div>

      <div class="about" id="about">
        <h3>About Geggets Stores</h3>
        <p>Geggets Stores — বাংলাদেশের একটি ডেমো ইলেকট্রনিক স্টোর। আমাদের লক্ষ্য মানসম্মত, কস্ট-এফেকটিভ গ্যাজেট সরবরাহ করা। এখানে আপনি Wearables, Lighting, Smart Home, Audio Devices ও Accessories—সব ধরনের স্মার্ট গ্যাজেট পাবেন।</p>
        <p><strong>Location:</strong> ঢাকা, মিরপুর, শাপলা প্লাজা, তৃতীয় তলা, ঢাকা</p>
        <p><strong>Categories:</strong> Wearables, Lighting, Audio, Smart Home, Accessories, Gaming</p>
        <p style="margin-top:10px"><strong>Open Chat:</strong> নিচের চ্যাট বক্সে সরাসরি আমাদের সাথে কথা বলতে পারো। কোনো সমস্যা বা অর্ডার সংক্রান্ত প্রশ্ন থাকলে এখানে মেসেজ করো — আমরা সাড়া দেব (ডেমো)।</p>
      </div>
      ...
      <div class="contact" id="contact">
        ...
        <p class="small">Address: ঢাকা, মিরপুর, শাপলা প্লাজা, তৃতীয় তলা, ঢাকা</p>
      ...
      <footer>© <span id="year"></span> Geggets Stores — All rights reserved.</footer>
    </main>
  </div>
  ...
  <div class="overlay" id="overlay">
    <div class="modal" role="dialog" aria-modal="true" id="review-modal">
      <div style="display:flex;justify-content:space-between;align-items:center">
        ...
        <button class="btn-secondary" onclick="closeReview()">✖</button>
      </div>
  ...
  <script>
    ...
    function onSearchInput(e){
      ...
      suggestionsBox.innerHTML = matches.map(m=>`<div class="suggestion-item" onclick="onSuggestionClick('${m.id}')">${m.name} — <span class="small" style="color:${'#6b7280'}">${m.desc}</span></div>`).join('');
      ...
      itemsDiv.innerHTML = cart.map((item,i)=>{
        ...
        return `
          <div class="cart-item">
            ...
            <div>
              <button class="btn-secondary" onclick="removeFromCart(${i})">❌</button>
            </div>
          </div>
        `;
      }).join('');
    ...
    function submitOrder(e){
      e.preventDefault();
      if(cart.length===0){ alert('কার্ট খালি — অর্ডার করা যাবে না।'); return; }
      const order = {
        id: 'ord_'+Date.now(),
    ...
    function openOrderHistory(){
      ...
      ordersList.innerHTML = orders.map(o=>{
        ...
        return `<div style="border-bottom:1px solid #f1f5f9;padding:8px 0;margin-bottom:6px">
          <div style="font-weight:700">${o.name} — <span class="small" style="font-weight:400">${date}</span></div>
    ...
    function submitCustomerService(e){
      ...
      document.getElementById('cs-feedback').textContent = '✅ Your message has been sent!';
    ...
    function postReview(){
      ...
      renderReviewSummaries();
      showReviewsInModal();
      alert('ধন্যবাদ — তোমার রিভিউ পোস্ট করা হয়েছে!');
    }
    function showReviewsInModal(){
      const pid = document.getElementById('review-modal').dataset.productId;
      const reviews = getReviews();
      const list = reviews[pid] || [];
      const out = list.map(r=>`<div class="review-item"><div style="font-weight:700">${'⭐'.repeat(r.rating)} <span class="small" style="font-weight:400"> - ${new Date(r.createdAt).toLocaleString()}</span></div><div class="small">${escapeHtml(r.comment||'')}</div></div>`).join('');
    ...
    function renderReviewSummaries(){
      PRODUCTS.forEach(p=>{
        ...
        if(list.length===0) container.innerHTML = `<div class="small">No reviews yet</div>`;
          ...
          container.innerHTML = `<div class="small" style="color:var(--muted)">Rating: ${avg} ⭐ — ${list.length} review(s)</div>`;
        }
      });
    }
    ...
    function sendChat(){
      ...
      // demo auto-response
      arr.push({ id:'c_'+(Date.now()+1), from:'support', text:'ধন্যবাদ আপনার মেসেজ জন্য — আমরা শীঘ্রই সাড়া দেব। (Demo reply)', createdAt:new Date().toISOString() });
